...
open class ARIScanView: UIView {
    
    // 扫码区域各种参数
    var viewStyle = ARIScanViewStyle()

    // 扫码区域
    var scanRetangleRect = CGRect.zero

    // 线条扫码动画封装
    var scanLineAnimation: ARIScanLineAnimation?

    // 网格扫码动画封装
    var scanNetAnimation: ARIScanNetAnimation?
    ...
    */
    public init(frame: CGRect, vstyle: ARIScanViewStyle) {
        viewStyle = vstyle

        switch viewStyle.anmiationStyle {
        case ARIScanViewAnimationStyle.LineMove:
            scanLineAnimation = ARIScanLineAnimation.instance()
        case ARIScanViewAnimationStyle.NetGrid:
            scanNetAnimation = ARIScanNetAnimation.instance()
        case ARIScanViewAnimationStyle.LineStill:
    ...
    // 根据矩形区域，获取识别区域
    static func getScanRectWithPreView(preView: UIView, style: ARIScanViewStyle) -> CGRect {
