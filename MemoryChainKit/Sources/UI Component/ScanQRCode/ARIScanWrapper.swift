...
open class ARIScanWrapper: NSObject,AVCaptureMetadataOutputObjectsDelegate, AVCapturePhotoCaptureDelegate {
    ...
    // 存储返回结果
    var arrayResult = [ARIScanResult]()

    // 扫码结果返回block
    var successBlock: ([ARIScanResult]) -> Void
    ...
    init(videoPreView: UIView,
         ...
         success: @escaping (([ARIScanResult]) -> Void)) {
    ...
    open func captureOutput(_ captureOutput: AVCaptureOutput,
        ...
        for current in metadataObjects {
            ...
            arrayResult.append(ARIScanResult(str: code.stringValue, img: UIImage(), barCodeType: code.type.rawValue, corner: code.corners as [AnyObject]?))
    ...
    public func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: Error?) {
        ...
        for idx in 0 ... self.arrayResult.count - 1 {
            self.arrayResult[idx].imgScanned = scanImage
        }
    }
    //MARK: ----拍照
    ...
    /**
     ...
     - returns: 返回识别结果
     */
    public static func recognizeQRImage(image: UIImage) -> [ARIScanResult] {
        ...
        return features.filter {
            ...
            ARIScanResult(str: $0.messageString,
                          img: image,
    ...
    static func getConcreteCodeImage(srcCodeImage: UIImage, codeResult: ARIScanResult) -> UIImage? {
    ...
    // 获取二维码的图像区域
    public static func getConcreteCodeRectFromImage(srcCodeImage: UIImage, codeResult: ARIScanResult) -> CGRect {
