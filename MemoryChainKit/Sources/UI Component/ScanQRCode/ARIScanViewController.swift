...
public protocol ARIScanViewControllerDelegate: AnyObject {
     func scanFinished(scanResult: ARIScanResult, error: String?)
}
...
open class ARIScanViewController: UIViewController {
    
    // 返回扫码结果，也可以通过继承本控制器，改写该handleCodeResult方法即可
    open weak var scanResultDelegate: ARIScanViewControllerDelegate?

    open var delegate: QRRectDelegate?

    open var scanObj: ARIScanWrapper?

    open var scanStyle: ARIScanViewStyle? = ARIScanViewStyle()

    open var qRScanView: ARIScanView?
    ...
    open override func viewDidAppear(_ animated: Bool) {
        ...
        drawScanView()
        perform(#selector(ARIScanViewController.startScan), with: nil, afterDelay: 0.3)
    }
    ...
    @objc open func startScan() {
        if scanObj == nil {
            ...
            if isOpenInterestRect {
                cropRect = ARIScanView.getScanRectWithPreView(preView: view, style: scanStyle!)
            }
            ...
            scanObj = ARIScanWrapper(videoPreView: view,
    ...
    open func drawScanView() {
        if qRScanView == nil {
            qRScanView = ARIScanView(frame: view.frame, vstyle: scanStyle!)
     ...
     */
    open func handleCodeResult(arrayResult: [ARIScanResult]) {
        ...
        if let result = arrayResult.first {
            ...
            let result = ARIScanResult(str: nil, img: nil, barCodeType: nil, corner: nil)
    ...
    @objc open func openPhotoAlbum() {
        ARIPermissions.authorizePhotoWith { [weak self] _ in
...
//MARK: - 图片选择代理方法
extension ARIScanViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    public func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey: Any]) {
        ...
        guard let image = editedImage ?? originalImage else {
            showAlert(title: nil, message: NSLocalizedString("Identify failed", comment: "Identify failed"))
            return
        }
        let arrayResult = ARIScanWrapper.recognizeQRImage(image: image)
...
//MARK: - 私有方法
private extension ARIScanViewController {
