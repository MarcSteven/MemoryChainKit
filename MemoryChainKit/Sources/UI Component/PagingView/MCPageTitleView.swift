...
@objc public protocol MCPagingTitleViewDelegate: NSObjectProtocol {
    /// 获取当前选中标题下标值
    @objc optional func pagingTitleView(titleView: MCPagingTitleView, index: Int)
}

public class MCPagingTitleView: UIView {
    @objc public init(frame: CGRect, titles:[String], configure: MCPagingTitleViewConfiguration) {
        super.init(frame: frame)
        
        backgroundColor = UIColor.white.withAlphaComponent(0.77)
        assert(!titles.isEmpty, "MCPagingTitleView 初始化方法中的配置信息必须设置")
    ...
    /// MCPagingTitleView 的代理
    @objc public weak var delegate: MCPagingTitleViewDelegate?
    ...
    private var configure: MCPagingTitleViewConfiguration!
    private var allBtnTextWidth: CGFloat = 0.0
    private var allBtnWidth: CGFloat = 0.0
    private var tempBtns = [MCPagingTitleButton]()
...
// MARK: 外部方法
public extension MCPagingTitleView {
    /// 根据 MCPagingContentView 子视图的滚动而去修改标题选中样式
    ...
    @objc func setImage(names: Array<String>, location: ImagePosition, spacing: CGFloat) {
        p_setImage(names: names, location: location, spacing: spacing)
    }
    
    /// 根据标题下标值设置标题图片及相对文字的位置（支持本地和网络图片）
    @objc func setImage(name: String, location: ImagePosition, spacing: CGFloat, index: Int) {
...
private extension MCPagingTitleView {
    /// 根据标题下标值重置标题文字
...
private extension MCPagingTitleView {
    ...
    func p_setImage(names: Array<String>, location: ImagePosition, spacing: CGFloat) {
        if tempBtns.count == 0 {
            return
        }
    ...
    func p_setImage(name: String, location: ImagePosition, spacing: CGFloat, index: Int) {
        if tempBtns.count == 0 {
            return
        }
...
// MARK: - 内部标题图片相关方法
private extension MCPagingTitleView {
    func setImage(btn: UIButton, imageName: String, location: ImagePosition, spacing: CGFloat) {
...
private class MCPagingTitleButton: UIButton {
