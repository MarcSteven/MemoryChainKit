...
class FPSStatusBarViewController: UIViewController {
    ...
    fileprivate let fpsMonitor = FPSMonitor()
...
extension FPSStatusBarViewController: FPSMonitorDelegate {

    @objc func fpsCounter(_ counter: FPSMonitor, didUpdateFramesPerSecond fps: Int) {
...
public extension FPSMonitor {
    ...
    @objc class func showInStatusBar(
        ...
        if let controller = window.rootViewController as? FPSStatusBarViewController {
            controller.fpsMonitor.startTracking(
                inRunLoop: runloop,
                mode: mode
            )
        }
    ...
    @objc class func hide() {
        ...
        if let controller = window.rootViewController as? FPSStatusBarViewController {
            controller.fpsMonitor.stopTracking()
