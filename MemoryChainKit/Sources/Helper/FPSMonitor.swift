...
///
/// `FPSMonitor` uses `CADisplayLink` updates to count the frames per second drawn.
...
/// `FPSMonitor.showInStatusBar()` convenience method.
///
public class FPSMonitor: NSObject {
    ...
    internal class DisplayLinkProxy: NSObject {
        ...
        @objc weak var parentMonitor: FPSMonitor?
    ...
    public override init() {
        ...
        self.displayLinkProxy.parentMonitor = self
    }
...
public protocol FPSMonitorDelegate: NSObjectProtocol {
    ...
    ///
    func fpsCounter(_ counter: FPSMonitor, didUpdateFramesPerSecond fps: Int)
}
