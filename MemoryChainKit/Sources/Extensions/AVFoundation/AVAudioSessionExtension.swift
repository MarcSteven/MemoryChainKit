...
@objc public extension AVAudioSession {
    ...
    func ChangeAudioOutput(_ presenterViewController : UIViewController, _ speakerButton: UIButton) {
        
        let CHECKED_KEY = "checked"
        var deviceAction = UIAlertAction()
        var headphonesExist = false
        
        if AudioOutputDeviceManager.shared.isDeviceListRequired() {
            ...
            if self.isBuiltInSpeaker {
                ...
                if AudioOutputDeviceManager.shared.isSpeaker {
                    let port = self.currentRoute.inputs.first!
                    setPortToNone(port)
                    AudioOutputDeviceManager.shared.isSpeaker = false
                }
            }
            else if self.isReceiver || self.isBuiltInMic  || self.isHeadphonesConnected {
                
                setPortToSpeaker()
                AudioOutputDeviceManager.shared.isSpeaker = true
            }
        }
    }
