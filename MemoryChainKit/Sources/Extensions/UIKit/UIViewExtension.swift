...
public extension UIView {
     ...
     */
    func screenshot(_ aspectRatio:CGFloat = 0,
                    offset:CGPoint? = nil,
                    quality:CGFloat = 1) ->UIImage? {
        var size :CGSize
        if aspectRatio > 0 {
            size = CGSize()
...
extension UIView {
    ...
    public func margin(to :UICoordinateSpace) ->UIEdgeInsets {
        ...
        return UIEdgeInsets(top: frame.minY, left: frame.minX, bottom: to.bounds.height - frame.maxY, right: to.bounds.width - frame.maxX)
    }
